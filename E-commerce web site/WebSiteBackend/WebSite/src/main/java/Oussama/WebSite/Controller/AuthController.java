...
public class AuthController {
    ...
    private void createAuthenticationToken(@RequestBody AuthenticationRequest authenticationRequest, HttpServletResponse response) throws IOException, JSONException {
       /*@RequestBody indique que les données de la requête HTTP doivent être converties en
       objet SignUpRequest
        */
        try {
    ...
    @PostMapping("/Sign-Up")
    /* la méthode signupUser qui accepte une requête HTTP POST. L'annotation @RequestBody
     indique que les données de la requête HTTP doivent être converties en un objet SignUpRequest.
      La méthode renvoie un objet ResponseEntity, qui est utilisé pour représenter la réponse HTTP.
     */
